...
void main() {
  testWidgets('HorizontalJigsawSchematic has correct padding', (
    WidgetTester tester,
  ) async {
    final segments = [
      Segment(
  ...
  testWidgets('HorizontalJigsawSchematic layouts multiple segments', (
    WidgetTester tester,
  ) async {
    final segments = [
      Segment(
    ...
    // left: 6, right: 2.0
    final segmentsWidgets = tester
        .widgetList<HorizontalJigsawSegment>(
          find.byType(HorizontalJigsawSegment),
        )
        .toList();
    final walkPadding = tester.widget<Padding>(
      find.descendant(
        of: find.byWidget(segmentsWidgets[0]),
        matching: find.byType(Padding),
      ),
    );
    expect(
      walkPadding.padding,
      const EdgeInsets.only(left: 6, right: 2.0, top: 1, bottom: 1),
    );
    ...
    expect(
      busPadding.padding,
      const EdgeInsets.only(left: 9.75, right: 6, top: 1, bottom: 1),
    );
  });

  testWidgets('HorizontalJigsawSchematic hides all walk labels', (
    WidgetTester tester,
  ) async {
    final segments = [
      Segment(
    ...
    // First segment (Walk)
    final row1 = find.descendant(
      of: find.byWidget(segmentsWidgets[0]),
      matching: find.byType(Row),
    );
    expect(
      find.descendant(of: row1, matching: find.text('Walk')),
      findsNothing,
    );
    expect(
      find.descendant(of: row1, matching: find.byType(Icon)),
      findsOneWidget,
    );

    // Third segment (Walk 2)
    final row3 = find.descendant(
      of: find.byWidget(segmentsWidgets[2]),
      matching: find.byType(Row),
    );
    expect(
      find.descendant(of: row3, matching: find.text('Walk')),
      findsNothing,
    );
    // Should still have icon
    expect(
      find.descendant(of: row3, matching: find.byType(Icon)),
      findsOneWidget,
    );
  });
}

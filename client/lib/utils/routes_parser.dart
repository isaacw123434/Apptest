    ...
    if (name.contains('Group 1') || name.contains('Group 2')) {
      ...
      if (options.isNotEmpty) {
        ...
        directDrive = DirectDrive(
          ...
          cost: _estimateCost('Direct Drive', totalDistMiles, [], routeId: routeId),
...
Leg _parseOptionToLeg(Map<String, dynamic> option, {String groupName = '', String? routeId}) {
  ...
  for (var jsonLeg in jsonLegs) {
    rawSegments.add(_parseSegment(jsonLeg, optionName: name, routeId: routeId));
  }
  ...
  for (var seg in mergedSegments) {
      if (seg.mode == 'train' && seg.from != null) {
          location = seg.from!.toLowerCase().split(' ')[0];
  ...
  for (int i = 0; i < mergedSegments.length - 1; i++) {
     ...
     if (current.mode == 'car') {
         ...
         for(int j=i+1; j<mergedSegments.length; j++) {
             if (mergedSegments[j].mode == 'train') {
                 connectsToTrain = true;
                 break;
             }
             if (mergedSegments[j].mode == 'bus' || mergedSegments[j].mode == 'car') {
                 break;
             }
         }
  ...
  // Inject Transfer Buffer for Route 2
  if (routeId == 'route2' &&
  ...
  // St Chads (Route 1) Logic Overrides
  if (routeId == 'route1') {
    // Drive to Leeds Station (Group 1) -> Add Parking £23
    if (groupName.contains('Group 1') && name.toLowerCase() == 'drive') {
        bool parkingExists = mergedSegments.any((s) => s.mode == 'parking');
        if (!parkingExists) {
            int carIndex = mergedSegments.lastIndexWhere((s) => s.mode == 'car');
            if (carIndex != -1) {
                mergedSegments.insert(carIndex + 1, Segment(
                     mode: 'parking',
                     label: 'Parking',
                     lineColor: '#000000',
                     iconId: IconIds.parking,
    ...
    if (name.toLowerCase() == 'uber' && groupName.contains('Group 1')) {
       for(int i=0; i<mergedSegments.length; i++) {
    ...
    if (name.contains('Walk') && name.contains('Train')) {
       bool trainApplied = false;
       for(int i=0; i<mergedSegments.length; i++) {
         if (mergedSegments[i].mode == 'train') {
           var seg = mergedSegments[i];
           mergedSegments[i] = Segment(
    ...
    if (name.contains('Uber') && name.contains('Train')) {
       bool uberApplied = false;
       bool trainApplied = false;
       for(int i=0; i<mergedSegments.length; i++) {
         if (mergedSegments[i].mode == 'car' || mergedSegments[i].label.contains('Uber')) {
            var seg = mergedSegments[i];
            mergedSegments[i] = Segment(
                  mode: seg.mode, label: seg.label, lineColor: seg.lineColor, iconId: seg.iconId, time: seg.time,
                  from: seg.from, to: seg.to, detail: seg.detail, path: seg.path, co2: seg.co2, distance: seg.distance,
                  cost: uberApplied ? 0.0 : 5.92
           );
           uberApplied = true;
         } else if (mergedSegments[i].mode == 'train') {
            var seg = mergedSegments[i];
            mergedSegments[i] = Segment(
                  mode: seg.mode, label: seg.label, lineColor: seg.lineColor, iconId: seg.iconId, time: seg.time,
                  from: seg.from, to: seg.to, detail: seg.detail, path: seg.path, co2: seg.co2, distance: seg.distance,
    ...
    if (name.toLowerCase().contains('bus')) {
       for(int i=0; i<mergedSegments.length; i++) {
...
Segment _parseSegment(Map<String, dynamic> jsonSegment, {String optionName = '', String? routeId}) {
...
double _estimateCost(String name, double distanceMiles, List<Segment> segments, {String? routeId}) {
    String lower = name.toLowerCase();
